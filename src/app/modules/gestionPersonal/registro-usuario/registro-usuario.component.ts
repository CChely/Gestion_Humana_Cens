...
export class RegistroUsuarioComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this._fuseMediaWatcherService.onMediaChange$
            ...
            .subscribe(({ matchingAliases }) => {
                this.drawerMode = matchingAliases.includes("md")
                    ? "side"
                    : "over";
    ...
    private _applyFilter(): void {
        ...
        this.filteredUsuarios = all.filter(
            (u) =>
    ...
    enterEditMode(): void {
        ...
        this._registroUsuarioService
            ...
            .subscribe({
                ...
                error: () => {
                    this.availableRoles = this.selectedUsuario!.roles.map(
                        (r) => ({
                            RolId: r.RolId,
                            Nombre: r.NombreRol,
    ...
    createUsuario(): void {
        if (this.newForm.invalid || this.newRolIds.size === 0) {
            return;
        }
    ...
    saveChanges(): void {
        if (this.editForm.invalid || !this.selectedUsuario) {
            return;
        }
        if (this.selectedRolIds.size === 0) {
            return;
        }
        ...
        const removeAvatar =
            ...
            !!this.selectedUsuario.avatar;

        // Always send avatar reference, empty string if not uploaded or being removed
        const avatarToSend = removeAvatar
            ? ""
            : this.avatarReference || this.selectedUsuario.avatar || "";
                ...
                next: (response) => {
                    ...
                    if (response?.status) {
                        ...
                        const updatedRoles: Rol[] = rolIds.map((id) => {
                            const cat = this.availableRoles.find(
                                (r) => r.RolId === id,
                            );
                            const existing = this.selectedUsuario!.roles.find(
                                (r) => r.RolId === id,
                            );
                            return {
                                RolId: id,
                                NombreRol:
                                    cat?.Nombre ?? existing?.NombreRol ?? "",
                                DescripcionRol:
                                    cat?.Descripcion ??
                                    existing?.DescripcionRol ??
                                    "",
                                FechaAsignacion:
                                    existing?.FechaAsignacion ??
    ...
    deleteUsuario(): void {
        ...
        confirmation.afterClosed().subscribe((result) => {
            ...
            this._registroUsuarioService
                .deleteUsuario(this.selectedUsuario!.UsuarioId)
                .pipe(takeUntil(this._unsubscribeAll))
                .subscribe({
                    next: (response) => {
                        this.isSaving = false;
        ...
        this._registroUsuarioService
            ...
            .subscribe({
                ...
                error: (err) => {
                    this.isUploadingAvatar = false;
                    this.errorMessage =
                        err?.error?.message || "Error al cargar la imagen";
                    this.selectedAvatarFile = null;
                    this.avatarPreviewUrl = null;
                    this.avatarReference = null;
                    this._changeDetectorRef.markForCheck();
